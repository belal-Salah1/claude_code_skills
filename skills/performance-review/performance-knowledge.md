...
>
> **Drift-guard:** this file is AUTHORITATIVE for performance calibration constants and domain laws; architecture laws and the anti-pattern catalog stay single-sourced in the architecture knowledge base (`.claude/docs/architecture-knowledge.md` where a project provides one) and are NOT duplicated here. Consuming skills address this file by **section number**, so renumbering, splitting, or inserting a section silently invalidates those pointers. On any change to the section structure here, grep `performance-knowledge.md` and update every consuming skill's inline `§` pointer (currently `performance-review/SKILL.md` Phase 0 → §3, Phase 2 → §1-2 + §10, Phase 3 → §4-§9, Phase 3 frontend rule → §7).
